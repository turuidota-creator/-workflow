...
async function testFetchRecords() {
    console.log('--- PocketBase Records Fetch Test ---');
    ...
    // 3. Fetch Records (Replicating exact server params)
    try {
        const collection = 'articles';
        const params = 'sort=-created&perPage=50';
        const fetchUrl = `${url}/api/collections/${collection}/records?${params}`;
        console.log(`\n[2] Fetching records: ${fetchUrl}`);
